...
async function fetchForPerson(person) {
  ...
  return parseRss(xml).slice(0, 10).map(item => {
    ...
    return {
      ...
      url: item.link,
      publishedDate: item.pubDate ? new Date(item.pubDate).toISOString().slice(0,10) : '',
...
export default async function handler(req) {
  ...
  if (req.method === 'GET') return json({ ok: true, version: '1.0', message: 'News Pulse backend is running' });
  ...
  if (people.length === 0) return json({ articles: [], note: 'No people to query.' });
  ...
  return json({
    articles,
    note: `Pulled ${articles.length} headlines for ${people.length} ${people.length === 1 ? 'person' : 'people'}${errors ? ` (${errors} failed)` : ''}.`,
  });
}
